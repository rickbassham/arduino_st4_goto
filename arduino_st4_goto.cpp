...
const char *ArduinoST4GoTo::getDefaultName()
{
    return "Arduino ST4 GoTo";
}

bool ArduinoST4GoTo::initProperties()
{
    ...
    IUFillNumber(&GuideRateN[RA_AXIS], "GUIDE_RATE_WE", "W/E Rate", "%g", 0, 2, 0.1, 2);
    IUFillNumber(&GuideRateN[DEC_AXIS], "GUIDE_RATE_NS", "N/S Rate", "%g", 0, 2, 0.1, 2);
...
void ArduinoST4GoTo::ISGetProperties(const char *dev)
{
...
bool ArduinoST4GoTo::updateProperties()
{
...
bool ArduinoST4GoTo::Connect()
{
    LOG_INFO("Arduino ST4 GoTo is online 2.");
    SetTimer(POLLMS);
...
bool ArduinoST4GoTo::Disconnect()
{
    sendCommand("DISCONNECT#");

    LOG_INFO("Arduino ST4 GoTo is offline.");

    return INDI::Telescope::Disconnect();
}

bool ArduinoST4GoTo::Handshake()
{
    return true;
}

bool ArduinoST4GoTo::ReadScopeStatus()
{
    // new axis control
    axisPrimary.update();
...
bool ArduinoST4GoTo::Goto(double r, double d)
{
    StartSlew(r, d, SCOPE_SLEWING);
    return true;
}

bool ArduinoST4GoTo::Sync(double ra, double dec)
{
    Angle a1, a2;
...
void ArduinoST4GoTo::StartSlew(double ra, double dec, TelescopeStatus status)
{
...
bool ArduinoST4GoTo::UnPark()
{
    SetParked(false);
    return true;
}

bool ArduinoST4GoTo::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
...
bool ArduinoST4GoTo::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
#ifdef USE_SIM_TAB
...
bool ArduinoST4GoTo::MoveNS(INDI_DIR_NS dir, TelescopeMotionCommand command)
{
...
bool ArduinoST4GoTo::MoveWE(INDI_DIR_WE dir, TelescopeMotionCommand command)
{
    if (TrackState == SCOPE_PARKED)
    {
        LOG_ERROR("Please unpark the mount before issuing any motion commands.");
...
IPState ArduinoST4GoTo::GuideNorth(uint32_t ms)
{
...
IPState ArduinoST4GoTo::GuideSouth(uint32_t ms)
{
    double rate = GuideRateN[DEC_AXIS].value;
    axisSecondary.StartGuide(-rate, ms);
    guidingNS = true;
...
IPState ArduinoST4GoTo::GuideEast(uint32_t ms)
{
...
IPState ArduinoST4GoTo::GuideWest(uint32_t ms)
{
    double rate = GuideRateN[RA_AXIS].value;
    axisPrimary.StartGuide(rate, ms);
    guidingEW = true;
...
{
    static_cast<ArduinoST4GoTo *>(p)->guideTimeout(ARD_N);
}
void ArduinoST4GoTo::guideTimeoutHelperS(void *p)
{
    static_cast<ArduinoST4GoTo *>(p)->guideTimeout(ARD_S);
}
void ArduinoST4GoTo::guideTimeoutHelperW(void *p)
{
    static_cast<ArduinoST4GoTo *>(p)->guideTimeout(ARD_W);
}
void ArduinoST4GoTo::guideTimeoutHelperE(void *p)
{
    static_cast<ArduinoST4GoTo *>(p)->guideTimeout(ARD_E);
}
...
bool ArduinoST4GoTo::SetDefaultPark()
{
...
bool ArduinoST4GoTo::SetTrackMode(uint8_t mode)
{
...
bool ArduinoST4GoTo::SetTrackEnabled(bool enabled)
{
...
bool ArduinoST4GoTo::SetTrackRate(double raRate, double deRate)
{
...
bool ArduinoST4GoTo::saveConfigItems(FILE *fp)
{
...
bool ArduinoST4GoTo::updateLocation(double latitude, double longitude, double elevation)
{
...
bool ArduinoST4GoTo::updateMountAndPierSide()
{
#ifdef USE_SIM_TAB
...
void ArduinoST4GoTo::guideTimeout(ARDUINO_DIRECTION direction)
{
